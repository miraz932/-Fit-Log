...
const SaveTotal = () => {
  const { saveState } = use(ExerciseContext)!
    const duration = saveState.reduce((acc,minutes)=>acc+ minutes.duration,0)
    const calories = saveState.reduce((acc,calories)=>acc+ calories.caloriesBurned,0)
  return (
    <div>
      ...
      <div className='flex justify-between px-6 py-5 pr-40 rounded-xl bg-[#13161D]'>
        ...
        <div className=''>
          ...
          <h1 className='font-bold text-4xl text-[#CCFF00]'>{saveState.length}</h1>
        </div>
